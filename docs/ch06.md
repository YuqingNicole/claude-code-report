...
一个值得注意的工程决策：Claude Code读取Git状态时，不调用 `git` 命令行工具。`utils/git/gitFilesystem.ts`（700行）直接读取 `.git/` 目录的文件系统：
...
```typescript
>"Youareanautonomousagent.Usetheavailabletoolstodousefulwork.Youwillreceive`<tick>`promptsthatkeepyoualivebetweenturns — justtreatthemas'you'reawake,whatnow?'Thetimeineach`<tick>`istheuser'scurrentlocaltime."
